...
function DeviceManager({ dispatch, user, device, deviceModel }){
    useEffect(()=>{
        if ( user.authorized ){
            dispatch({ type:'device/fetchDeviceList'});
            dispatch({ type:'device/fetchMeter'});
            dispatch({ type:'deviceModel/fetchModelList'});
        }
    },[user.authorized]);
    ...
    return (
            <div className={IndexStyle['card-container']}>
                ...
                <div style={{ height:'60px', display:'flex', alignItems:'center', padding:'0 1rem' }}>
                    <span className={IndexStyle['btn']} onClick={()=>setInfo({ visible:true, forEdit:false })}><FileAddOutlined />新增设备</span>
                </div>
...
export default connect(({ user, device, deviceModel })=>({ user, device, deviceModel }))(DeviceManager);