    ...
    return (
        <div className={style['container'] + ' ' + ( theme === 'dark' ? style['dark'] : '')}>
            ...
            <div style={{ display:'inline-flex'}}>
                <div className={ size === 'small' ? style['date-picker-button-left'] + ' ' + style['small'] : style['date-picker-button-left']} onClick={()=>{
                    let start,end ;
                ...
                }}><LeftOutlined /></div>
                {
                    timeType === '1' 
                    ?
                    <DatePicker ref={inputRef} size={ size || 'default'} locale={zhCN} allowClear={false} className={ size==='small' ? style['date-picker-container'] + ' ' + style['small'] : style['date-picker-container'] } value={startDate} onChange={value=>{
                    ...
                    :
                    <RangePicker ref={inputRef} size={size || 'default'} locale={zhCN} allowClear={false} className={size === 'small' ? style['date-picker-container'] + ' ' + style['small'] : style['date-picker-container']} value={[startDate, endDate]} onChange={arr=>{
                ...
                <div className={ size === 'small' ? style['date-picker-button-right'] + ' ' + style['small'] : style['date-picker-button-right']} onClick={()=>{
                    let start,end;
                    let temp = new Date(startDate.format('YYYY-MM-DD'));
                    if ( timeType === '1'){
