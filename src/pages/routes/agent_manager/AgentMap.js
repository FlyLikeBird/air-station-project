...
function AgentMap({ companyList, msg, AMap, currentNode, userId, dispatch }) {
    ...
    useEffect(()=>{
        if ( AMap ){
            ...
            function handleShowInfo(e){
                clearTimeout(timer);
                let target = e.target;
                let { lng, lat, company_name, company_id, gasDevice, gasStation } = target._originOpts.extData;
                let pos = map.lngLatToContainer(new AMap.LngLat(lng, lat));
                setInfo({ x:pos.x, y:pos.y, company_name, company_id, gasDevice, gasStation });
                timer = setTimeout(()=>{
                    setInfo({});
                },2000)                    
            }
            ...
            companyList.forEach(item=>{
                ...
                let marker = new AMap.Marker({
                    ...
                    icon:arrowNormal,
                    extData:{ province:item.province, city:item.city, area:item.area, lng:+item.lng, lat:+item.lat, company_name:item.company_name, company_id:item.company_id, gasDevice:item.gasDevice, gasStation:item.gasStation }
                });
                map.add(marker);
                points.push(marker);
    ...
    return (
        <div style={{ height:'100%' }}>
            <div className={style['info-container']} style={{ display: Object.keys(info).length ? 'block' : 'none', top: ( info.y - 160 ) + 'px', left: ( info.x - 80 ) + 'px' }}>
                <div className={style['info-title']}>{info.company_name}</div>
                <div className={style['info-content']}>
                    <div>
                        <div style={{ color:'rgba(255,255,255,0.64)', fontSize:'0.8rem' }}>空压站</div>
                        <div><span className={style['data']}>{ info.gasStation }</span><span className={style['unit']}>个</span></div>
                    </div>
                    <div>
                        <div style={{ color:'rgba(255,255,255,0.64)', fontSize:'0.8rem' }}>空压机</div>
                        <div><span className={style['data']} style={{ color:'#f30d0d' }}>{ info.gasDevice }</span><span className={style['unit']} style={{ color:'#f30d0d' }}>个</span></div>
                    </div>
                </div>
                <div style={{ textAlign:'center' }}><span className={style['btn']} onClick={()=>{ 
                    dispatch({ type:'user/resetUserInfo' });
                    history.push(`/?pid=${Math.random()}&&userId=${userId}&&companyId=${info.company_id}&&mode=full`);
                }}>进入项目</span></div>
            </div>
